...
clf_split = MLPClassifier(random_state=42, max_iter=1000)
clf_split.fit(X_train, y_train)
y_pred = clf_split.predict(X_test)
accuracy_split = accuracy_score(y_test, y_pred)
print(f"Train-Test Split Accuracy (MLP Classifier): {accuracy_split * 100:.2f}%")

# 10-Fold Cross Validation
clf_cv = MLPClassifier(random_state=42, max_iter=1000)
cv_scores = cross_val_score(clf_cv, X, y, cv=10)
print("\n10-Fold Cross-Validation Accuracies (MLP Classifier):")
